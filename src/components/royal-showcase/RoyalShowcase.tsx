...
export default function RoyalShowcase() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (Array.isArray(data.featuredProducts) && data.featuredProducts.length > 0) {
            ...
            const mapped: ProductWithMetadata[] = [];
            ...
            for (const fp of data.featuredProducts) {
              ...
              if (baseProd) {
                mapped.push({
                  ...
                  badgeType: fp.badgeType || (baseProd.isBestSeller ? 'BEST_SELLER' : 'FEATURED'),
  ...
  // Gracefully handle empty state by hiding section when zero products exist
