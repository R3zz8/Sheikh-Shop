...
export default function GoogleAuthButton({ onClick, isLoading: externalIsLoading, disabled }: GoogleAuthButtonProps) {
    ...
    return (
        <div className="w-full">
            <motion.button
                ...
                className="w-full h-[52px] inline-flex items-center justify-center gap-3 rounded-2xl border border-white/[0.08] bg-neutral-950/45 text-slate-200 hover:text-amber-400 transition-all duration-300 shadow-lg font-vazirmatn text-sm sm:text-base font-semibold px-4 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
                {isLoading ? (
                    <svg className="animate-spin h-5 w-5 text-amber-500" fill="none" viewBox="0 0 24 24" aria-hidden="true">
                ...
                <span className="font-bold">
                    {isLoading ? "در حال اتصال به گوگل..." : "ادامه با حساب گوگل"}
                </span>
            </motion.button>
