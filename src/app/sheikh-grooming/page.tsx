...
// Import custom Sheikh Grooming page sections
import SheikhGroomingEmptyState from '@/components/sheikhGrooming/SheikhGroomingEmptyState';
...
/**
 * Generate metadata for the Sheikh Grooming page
 */
export async function generateMetadata(): Promise<Metadata> {
  const baseUrl = getBaseUrl();
  const canonicalPath = '/sheikh-grooming';
  const canonicalUrl = `${baseUrl}${canonicalPath}`;

  return {
    title: 'شیخ گرومینگ | فروشگاه بزرگ شیخ',
    description: 'مجموعه‌ای انحصاری از ماشین‌های اصلاح مو، تریمرهای حرفه‌ای، موزر و تجهیزات آراستگی شخصی در فروشگاه بزرگ شیخ.',
    keywords: [
      'شیخ گرومینگ',
      'ماشین اصلاح موزر',
      'تریمر خط زن',
      'ماشین اصلاح صورت',
      'تجهیزات اصلاح پیرایش',
      'sheikh grooming',
      'hair clipper',
      'beard trimmer',
    ],
    openGraph: {
      title: 'شیخ گرومینگ | فروشگاه بزرگ شیخ',
      description: 'مجموعه گزینش‌شده و لوکس تجهیزات حرفه‌ای اصلاح، پیرایش و آراستگی شخصی.',
      type: 'website',
      url: canonicalUrl,
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: 'مجموعه تجهیزات گرومینگ و اصلاح شیخ - فروشگاه شیخ',
        },
      ],
    },
    twitter: {
      ...
      title: 'شیخ گرومینگ | فروشگاه بزرگ شیخ',
      description: 'کامل‌ترین مجموعه تجهیزات اصلاح و آراستگی حرفه‌ای فروشگاه شیخ.',
...
export default async function SheikhGroomingPage() {
  try {
    const [data, units] = await Promise.all([
      getProductsByCategory(ProductCategoryType.SheikhGrooming),
    ...
    if (!data || !Array.isArray(data)) {
      console.error('Invalid grooming products list format received');
    ...
    const breadcrumbs = [
      ...
      { name: 'شیخ گرومینگ', url: '/sheikh-grooming' },
    ];
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-[#0a0503] via-[#120703] to-[#0a0503] text-white">
        ...
        <div id="grooming-products-section" className="scroll-mt-24 pb-16">
          {serializedProducts.length > 0 ? (
            <ProductListView
              products={serializedProducts}
              units={serializedUnits}
              title="شیخ گرومینگ"
              subtitle="مجموعه‌ای از بهترین ماشین‌های اصلاح، تریمرها و تجهیزات پیرایش شخص لایف استایل لوکس."
              variant="grooming"
            />
          ) : (
            <SheikhGroomingEmptyState />
          )}
        </div>
      </div>
    );
    ...
    console.error('Failed to load Sheikh Grooming page:', {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-stone-950 font-vazirmatn" dir="rtl">
        <div className="text-center p-8 border border-amber-500/10 bg-[#140b07]/80 rounded-[2rem] max-w-2xl mx-auto shadow-2xl">
          ...
          <h1 className="text-xl font-bold text-white mb-2">خطا در بارگذاری شیخ گرومینگ</h1>
          <p className="text-gray-400 text-sm mb-6">
            در بارگذاری صفحه شیخ گرومینگ مشکلی موقت پیش آمده است. لطفا صفحه را مجددا بارگذاری نمایید.
          </p>
