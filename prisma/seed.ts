...
async function main() {
  ...
  const categories = await Promise.all([
    ...
    prisma.category.upsert({
      where: { slug: 'sheikh-nicotine' },
      update: {
        name: 'شیخ نیکوتین',
        description: 'مجموعه تخصصی ویپ، پاد سیستم و محصولات نیکوتین لوکس شیخ',
        image: '/nicotine.webp',
        isActive: true,
        sortOrder: 5,
      },
    ...
    prisma.category.upsert({
      where: { slug: 'sheikh-grooming' },
      update: {
        name: 'شیخ گرومینگ',
        description: 'تجهیزات مدرن و حرفه‌ای اصلاح و پیرایش شخصی شیخ گرومینگ',
        image: '/grooming.webp',
        isActive: true,
        sortOrder: 6,
      },
  ...
  const newProducts = await Promise.all([
    ...
    // Sheikh Nicotine Sample Products
    prisma.product.upsert({
      where: { name: 'پاد سیستم لوکس شیخ مدل Royal Vape X1' },
      update: {},
      create: {
    ...
    prisma.product.upsert({
      where: { name: 'ای-لیکوئید سالت نیکوتین شیخ مدل Golden Mango 30ml' },
      update: {},
      create: {
    ...
    // Sheikh Grooming Sample Products
    prisma.product.upsert({
      where: { name: 'ماشین اصلاح حرفه‌ای سر و صورت شیخ مدل Clipper Pro 9000' },
      update: {},
      create: {
    ...
    prisma.product.upsert({
      where: { name: 'تریمر خط‌زن و اصلاح ریش شیخ مدل Precision Gold' },
      update: {},
      create: {
